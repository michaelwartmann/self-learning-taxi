...
def test_reporter():
    plt.ion()
    rep = Reporter(metrics=['net_reward', 'avg_net_reward', 'final_deadline', 'success'], live_plot=True)
    for i in xrange(100):
        rep.collect('net_reward', i, self.env.trial_data['net_reward'])  # total reward obtained in this trial
        rep.collect('avg_net_reward', i, np.mean(self.rep.metrics['net_reward'].ydata[-self.avg_net_reward_window:]))  # rolling mean of reward
        rep.collect('final_deadline', i, self.env.trial_data['final_deadline'])  # final deadline value (time remaining)
        rep.collect('success', i, self.env.trial_data['success'])
        time.sleep(0.01)
    rep.plot()
    summary = rep.summary()
